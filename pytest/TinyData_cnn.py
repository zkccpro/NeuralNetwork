...
def ut_TinyData_cnn():
    ...
    dataset = dataSet.CustomedDataSet(train_x=pic_list, train_y=label,
                                      ...
                                      data_transform=globalParam.pic_data_transfer, target_data_transform=globalParam.num_data_transfer)
    ...
    action.train(cnn, dataloader, testloader,
                globalParam.bottleneckcnn_optimizer, reg_inferencer,
                # scheduler=globalParam.scheduler, warmup_scheduler=globalParam.warmup_scheduler,
                iteration_show=100, max_epoch=10)
