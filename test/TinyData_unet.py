...
def ut_TinyData_unet():
    ...
    # 2w张图片，训练时稳定占用25G内存
    pic_list, test_list, label, test_label = dataReader.UnetDataReader(r"data/TinyData/", r"data/DiffPic/").read(50, 42)
    ...
    action.train(cnn, dataloader, testloader, globalParam.unet_optimizer, epoch=5)
    ...
    # print('--------------------------------------------')
    # print('saving model!')
    # action.save_model(cnn, 'model/Unet/unet_batch=21_epoch=100_lr=0.00005_momentum=0.9_weight_decay=1e-2')
    #
    # # playback
    # print('--------------------------------------------')
    # print('playback model!')
    # playback_cnn = model.unet_network()  # 这里记得改！
    # action.model_playback(playback_cnn, 'model/Unet/unet_batch=21_epoch=100_lr=0.00005_momentum=0.9_weight_decay=1e-2')
